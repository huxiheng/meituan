...
{
	self = [super init];
	[self setShouldRespectCacheControlHeaders:YES];   //默认 要检查服务器返回的缓存控制字段
...
//检查该request的相应头来判断 服务器是否允许缓存
	if ([self shouldRespectCacheControlHeaders] && ![[self class] serverAllowsResponseCachingForRequest:request]) {
		[[self accessLock] unlock];
		return;
	}

	NSString *headerPath = [self pathToStoreCachedResponseHeadersForRequest:request];//得到存储response header的路径
	NSString *dataPath = [self pathToStoreCachedResponseDataForRequest:request];    //得到存储response data的路径

	NSMutableDictionary *responseHeaders = [NSMutableDictionary dictionaryWithDictionary:[request responseHeaders]]; //获取响应头
	if ([request isResponseCompressed]) {                                           //去掉响应头中的Content-Encoding字段
    ...
    /**
     *添加X-ASIHTTPRequest-Expires  头字段，用于判断该response是否过期
     */
	...
	[responseHeaders writeToFile:headerPath atomically:NO];                 //存储response headers

	if ([request responseData]) {                                           //存储response 数据
...
//根据request 的 cacheStoragePolicy 来生成存储路径， 只有两种一种是再/Library/Caches/PermanentStore 另一个是/Library/Caches/SessionStore
