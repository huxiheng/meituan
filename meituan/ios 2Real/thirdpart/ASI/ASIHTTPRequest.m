...
+ (void)initialize
{
	if (self == [ASIHTTPRequest class]) {
		...
		ASIRequestTimedOutError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:ASIRequestTimedOutErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:NSLocalizedString(@"The request timed out", @"The request timed out") ,NSLocalizedDescriptionKey,nil]];
		ASIAuthenticationError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:ASIAuthenticationErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys: NSLocalizedString(@"Authentication needed", @"Authentication needed") ,NSLocalizedDescriptionKey,nil]];
		ASIRequestCancelledError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:ASIRequestCancelledErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:NSLocalizedString(@"The request was cancelled", @"The request was cancelled"),NSLocalizedDescriptionKey,nil]];
		ASIUnableToCreateRequestError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:ASIUnableToCreateRequestErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:NSLocalizedString(@"Unable to create request (bad url?)",@"Unable to create request (bad url?)" ),NSLocalizedDescriptionKey,nil]];
		ASITooMuchRedirectionError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:ASITooMuchRedirectionErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:NSLocalizedString(@"The request failed because it redirected too many times", @"The request failed because it redirected too many times") ,NSLocalizedDescriptionKey,nil]];
...
{
	// Add cookies from the persistent (mac os global) store //如果使用userCookiePersistence   从NSHTTPCookieStorage中获取
...

{
	...
	if (![self error]) { // We may already have handled this error
		...
		[self cancelLoad];
		[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:ASIConnectionFailureErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:NSLocalizedString(reason, reason),NSLocalizedDescriptionKey,underlyingError,NSUnderlyingErrorKey,nil]]];
	} else {
		[self cancelLoad];
	}
	[self checkRequestStatus];
...
+ (NSDate *)expiryDateForRequest:(ASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
    ...
    //到这里如果maxAge 有效 (用户指定或者根据response header生成)
	if (maxAge) {
